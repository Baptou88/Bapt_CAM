...
class HoleRecognitionTaskPanel:
    ...
    def create_ui(self):
        ...
        self.dia_tolerance_spin.valueChanged.connect(self.on_dia_tolerance_changed)
    ...
    def update_results(self):
        ...
        self.holes_table.setRowCount(len(self.obj.Proxy.detected_holes))
        
        for row, hole in enumerate(self.obj.Proxy.detected_holes):
            # Position
            pos_str = f"({hole.center.x:.2f}, {hole.center.y:.2f}, {hole.center.z:.2f})"
            self.holes_table.setItem(row, 0, QtGui.QTableWidgetItem(pos_str))
            
            # Diamètre
            dia_str = f"{hole.diameter:.2f} mm"
            self.holes_table.setItem(row, 1, QtGui.QTableWidgetItem(dia_str))
            
            # Profondeur
            depth_str = f"{hole.depth:.2f} mm"
            self.holes_table.setItem(row, 2, QtGui.QTableWidgetItem(depth_str))
            ...
            self.holes_table.setItem(row, 3, QtGui.QTableWidgetItem(group_str))
        
        # Mettre à jour la table des groupes
        self.groups_table.setRowCount(len(self.obj.Proxy.hole_groups))
        
        for row, group in enumerate(self.obj.Proxy.hole_groups):
            # Diamètre
            dia_str = f"{group.diameter:.2f} mm"
            self.groups_table.setItem(row, 0, QtGui.QTableWidgetItem(dia_str))
            
            # Profondeur
            depth_str = f"{group.depth:.2f} mm"
            self.groups_table.setItem(row, 1, QtGui.QTableWidgetItem(depth_str))
            
            # Quantité
            qty_str = str(group.count())
            self.groups_table.setItem(row, 2, QtGui.QTableWidgetItem(qty_str))
            ...
            self.groups_table.setCellWidget(row, 3, action_widget)
