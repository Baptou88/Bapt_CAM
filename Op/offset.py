...
def reverse_toolpath_for_mode(machining_mode: str) -> bool:
...
def offsetWire(wire: Part.Wire, distance: float, forward: bool, side: Side = Side.NONE) -> Part.Wire:
    ...
    if len(wire.Edges) == 1:
        ...
        if isinstance(curve, Part.Line) or isinstance(curve, Part.LineSegment):
            ...
            App.Console.PrintMessage(f'Offsetting line  {side} \n')
            ...
            if (forward and side == Side.RIGHT) or (not forward and side == Side.LEFT):
    ...
    if wire.isClosed():
        if side == Side.INSIDE or side == Side.LEFT:
    ...
    wire_offset = wire.makeOffset2D(offset_sign_from_material_side(side) * distance, join=0, fill=False, openResult=True)
    return wire_offset
    pass
